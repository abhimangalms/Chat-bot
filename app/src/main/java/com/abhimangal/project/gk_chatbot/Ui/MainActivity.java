...
import com.abhimangal.project.gk_chatbot.Adapters.MessageAdapter;
import com.abhimangal.project.gk_chatbot.Contract.MessageContract;
import com.abhimangal.project.gk_chatbot.DBHelper.MessageDBHelper;
import com.abhimangal.project.gk_chatbot.DataTypes.MessageData;
import com.abhimangal.project.gk_chatbot.R;
import com.abhimangal.project.gk_chatbot.UtilityPackage.Constants;
...
//        });
    ...
    private void logoutUser() {
        // ask for user confirmation
        AlertDialog.Builder builder = new AlertDialog.Builder(MainActivity.this);
        builder.setMessage("Do you want to logout from GK ChatBot?");
        builder.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
    ...
    public void sendChatMessage() throws UnsupportedEncodingException {
        ...
        else if (message.toUpperCase().startsWith("WHAT")) {
            // search response, requested by user

            String[] messageArray = message.toLowerCase().split(" ");

            if (Arrays.asList(messageArray).contains("car")) {

                String[] carResult = {
                        "It's a four-wheeled road vehicle that is powered by an engine and is able to carry a small number of people.",
                };

                displayBotReply(new MessageData(Constants.BOT, carResult[new Random().nextInt(carResult.length)] + "\n" + " ", timeStamp));

            } else if (Arrays.asList(messageArray).contains("bike")) {

                String[] bikeResult = {
                        "A bicycle, also called a bike or cycle, is a human-powered or motor-powered, pedal-driven, single-track vehicle, having two wheels attached to a frame, one behind the other. A bicycle rider is called a cyclist, or bicyclist.",
                };

                displayBotReply(new MessageData(Constants.BOT, bikeResult[new Random().nextInt(bikeResult.length)] + "\n" + " ", timeStamp));

            } else if (Arrays.asList(messageArray).contains("school")) {

                String[] schoolResult = {
                        "A school is an educational environment where people, particularly young children go to learn about topics such as reading, writing and mathematics from a teacher. Most of a student's time will be spent in a classroom, a place where usually 10 to 30 people sit in to engage in educational discussion.",
                };

                displayBotReply(new MessageData(Constants.BOT, schoolResult[new Random().nextInt(schoolResult.length)] + "\n" + " ", timeStamp));

            } else {
                String[] googleResult = {"OOPS!!"};

                displayBotReply(new MessageData(Constants.BOT, googleResult[new Random().nextInt(googleResult.length)] + "\n" + " Searching google result for " + message, timeStamp));
                Toast.makeText(MainActivity.this, "Opening browser", Toast.LENGTH_SHORT).show();

                String escapedQuery = URLEncoder.encode(message, "UTF-8");
        ...
        /** QA with WHO*/
        ...
        else if (message.toUpperCase().startsWith("WHO")) {
            // search response, requested by user

            String[] messageArray = message.toLowerCase().split(" ");

            if (Arrays.asList(messageArray).contains("dhoni")) {
            ...
            } else if (Arrays.asList(messageArray).contains("messi")) {
            ...
            } else if (Arrays.asList(messageArray).contains("minister")) {
        ...
        /** QA with WHERE*/
        ...
        else if (message.toUpperCase().startsWith("WHERE")) {
            // search response, requested by user

            String[] messageArray = message.toLowerCase().split(" ");

            if (Arrays.asList(messageArray).contains("tajmahal")) {
            ...
            } else if (Arrays.asList(messageArray).contains("ladakh")) {
            ...
            } else if (Arrays.asList(messageArray).contains("sriharikota")) {
